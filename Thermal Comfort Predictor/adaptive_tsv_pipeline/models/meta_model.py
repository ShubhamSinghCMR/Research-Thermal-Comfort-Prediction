"""
Adaptive Meta-Model Module for Thermal Comfort Prediction
==========================================================

This module implements the adaptive meta-model (stacking) layer of the thermal comfort prediction system.
It uses LightGBM as the meta-learner to combine predictions from base models with support for dynamic features.

Key Features:
-------------
- LightGBM meta-learner with adaptive environment-specific parameters
...
from adaptive_tsv_pipeline.models.base_models import evaluate_predictions
from adaptive_tsv_pipeline.utils.config import SHOW_WARNINGS
...
def train_meta_model_kfold(oof_preds, y, env_params, n_splits=5):
    ...
    # Ensure verbosity is handled based on config
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(oof_preds, y), 1):
        ...
        if SHOW_WARNINGS:
            print(
                f"[Meta-model Fold {fold}] "
                f"RMSE={metrics['RMSE']:.3f}, MAE={metrics['MAE']:.3f}, R2={metrics['R2']:.3f}"
            )

    # Aggregate metrics and feature importance
    ...
    if SHOW_WARNINGS:
        print(
            f"\n[Meta-model] Final CV Metrics: "
            f"RMSE={avg_metrics['RMSE']:.3f}, MAE={avg_metrics['MAE']:.3f}, R2={avg_metrics['R2']:.3f}"
        )
